    ...
    public void execute(CommandContext context) {
        ...
        stub.observeEvents(Empty.getDefaultInstance())
                .forEachRemaining(event -> {
                    if (event.getEventCase() == com.netflix.titus.grpc.protogen.SupervisorEvent$EventCase.MASTERINSTANCEUPDATE) {
                        logger.info("Add/updated: {}", SupervisorGrpcModelConverters.toCoreMasterInstance(event.getMasterInstanceUpdate().getInstance()));
                    } else if (event.getEventCase() == com.netflix.titus.grpc.protogen.SupervisorEvent$EventCase.MASTERINSTANCEREMOVED) {
