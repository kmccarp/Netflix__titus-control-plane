        ...
        private StateHolder(S state, String reason) {
            ...
            if (!finalStateEval.apply(state)) {
                PolledMeter.using(registry).withId(this.currentStateId).monitorValue(this,
                        ...
                        self -> {
                            if (self.lifecycle == com.netflix.titus.common.util.spectator.FsmMetricsImpl$StateHolderLifecycle.Active) {
                                return 1;
                            } else if (self.lifecycle == com.netflix.titus.common.util.spectator.FsmMetricsImpl$StateHolderLifecycle.Inactive) {
